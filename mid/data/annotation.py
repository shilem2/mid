...
sitk.ProcessObject_SetGlobalWarningDisplay(False)  # suppress warnings
...
class Annotation(MutableMapping):
    ...
    def values(self, order='xy', units=None, keys=None, s1_upper_only=True):
        ...
        if (s1_upper_only == False) or ('S1' not in keys):
...
def plot_ann(fig, ann, text, color='r', marker='x', markersize=3, fontsize=6, plot_lines=True, plot_text=True, linestyle='-', linewidth=0.75):
    ...
    if plot_lines:
        for n in range(0, ann.shape[0], 2):  # jump 2 rows at a time
            ax.plot(ann[n:n + 2, 0], ann[n:n + 2, 1], color=color, linestyle=linestyle, linewidth=linewidth)
